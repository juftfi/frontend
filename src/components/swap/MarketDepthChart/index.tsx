...
const MarketDepthChart = ({ currencyA, currencyB, isOpen, close, poolAddress }: MarketDepthChartProps) => {
    ...
    const tooltipY = useMemo(() => {
        ...
        const id = document.getElementById(
            isAfterCurrent(hoveredIndex) ? `before-current-${hoveredIndex}` : `after-current-${-hoveredIndex}`
        );
    ...
    const [highestTick, lowestTick] =
        beforeCurrent && afterCurrent ? [beforeCurrent[0].tick, afterCurrent[afterCurrent.length - 1].tick] : [];
    ...
    return (
        <div
            className={`h-[100vh] lg:h-full bg-card fixed border-l border-card-border right-0 top-0 left-0 bottom-0 lg:left-[unset] overflow-x-hidden lg:overflow-x-visible duration-200 z-[99] ${
                isOpen ? "w-[100vw] md:w-[380px]" : "w-[0px]"
            }`}
        >
            <div className="flex flex-col lg:flex-row w-full h-full">
                <button className="h-full z-[99] p-1 bg-card-dark border-x border-card-border hover:bg-card-hover" onClick={close}>
                                  ...
                                  >
                                      <div className={`absolute flex justify-evenly w-full`}>
                                          <div className="w-full text-left -ml-12 text-red-500 font-semibold">
                                              {formatCurrency.format(invertPrice ? v.price1 : v.price0)}
                                          </div>
                                          <div className="w-full">{formatCurrency.format(invertPrice ? v.tvlToken1 : v.tvlToken0)}</div>
                                          <div className="w-full text-right pr-4">
                                              {formatCurrency.format(invertPrice ? v.tvlToken0 : v.tvlToken1)}
                                          </div>
                                      </div>
                                      <div
                                          key={`before-price-${idx}`}
                                          className={`h-[20px] rounded-l-lg ml-auto ${
                                              idx >= hoveredIndex &&
                                              hoveredIndex >= 0 &&
...
const MarketDepthChartTooltip = ({ currencyA, currencyB, tooltipData }: any) => {
